...
static int updateAvlTreeNodeHeight(AVLTreeNode *node)
{
    ...
    node->height = 1 + temMAX(leftheight, rightheight);
...
static int minMax(int val1, int val2)
{
...
static int updateAvlTreeNodeHeight(AVLTreeNode *node)
{
    ...
    int leftHeight = (node->left == NULL) ? 0 : node->left->height;
    int rightHeight = (node->right == NULL) ? 0 : node->right->height;
    node->height = 1 + minMax(leftHeight, rightHeight);
    return ret;
}
/* 获取AVL树结点的平衡因子 */
static int avlTreeNodeGetFactor(AVLTreeNode *node)
...
#else
    int nodeFactor = absAVLTreeNodeBalanceFactor(node);
    if (nodeFactor <= 1)
    {
        return true;
    }
